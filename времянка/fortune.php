...
//это что б не выскакивала ошибка поскольку браузеру запрещено отправлять на стр без этого заголовка
$name = mb_convert_encoding($_POST["name"],'UTF-8');//это берется из ajax(data:"тут че надо передавать")
...
$counter = 1;
$lenght_name = iconv_strlen($name);//возвращает кол-во символов в строке учитывая кодировку. Длина строки не обязательно будет соответствовать количеству байт в ней.
